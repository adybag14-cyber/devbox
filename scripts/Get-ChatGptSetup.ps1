...
$runtimeLabel = if ($response.runtime_mode) { [string]$response.runtime_mode } else { 'auto' }

Write-Host "Name: Devbox MCP"
Write-Host "Description: Runtime-aware Devbox MCP ($runtimeLabel) with optional native host tools"
