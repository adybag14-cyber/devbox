...
Assert-Contains $ownership '$runtimeEnvMatches = [IO.Path]::IsPathRooted($runtimePath)' 'Checkout-scoped ownership must reject relative runtime-env paths.'
Assert-Contains $ownership 'if ([IO.Path]::IsPathRooted($argument)' 'Checkout-scoped ownership must reject relative server paths.'
