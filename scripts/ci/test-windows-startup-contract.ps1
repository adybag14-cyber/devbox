...
Assert-Contains $start '$value = if ([string]::IsNullOrWhiteSpace($ConfiguredValue)) { ''cpp'' }' 'C++ must be the managed MCP default when no implementation override is configured.'
