...
Assert-Contains $start '& $cargoExe ''build'' ''--manifest-path'' $manifestPath ''--release'' ''--locked''' 'Rust replacement preflight must build the locked release binary.'
...
Assert-Contains $start '$isRust = $text.Contains(''rust-mcp\target\release\devbox-mcp.exe'')' 'Managed ownership checks must recognize the Rust MCP binary.'
