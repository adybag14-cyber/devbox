...
Linux x86_64 and arm64 use an OS-owned bubblewrap dependency, separate user/PID/network/IPC/UTS namespaces, an otherwise read-only filesystem, a private workspace and temporary directory, dropped capabilities, no-new-privileges, and a native seccomp filter. Sockets and process creation are denied; ordinary threads remain usable. The native bootstrap applies address-space, CPU, file-size and descriptor limits before executing bubblewrap. No credential or host directory descriptor is passed to the untrusted program. Namespace-permission failures never fall back to ordinary execution. Aggregate workspace disk usage is not an OS hard quota in this profile.

A real x86_64 Ubuntu/WSL qualification with bubblewrap 0.6.1 and GCC 12.3 passed namespace/seccomp activation, secret denial, network and fork denial, ordinary threads, a writable workspace, a read-only namespace root and descriptor-leak checks. arm64 hosted qualification remains pending. Unqualified platforms explicitly refuse the profile. Broader cancellation/adversarial coverage, provider/run-controller integration, and complete hosted qualification remain H04/H12-H14 work. None of this checkpoint has been promoted to production.
