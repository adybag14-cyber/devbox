...
## Structured WSL checkpoint

Contract 9 exposes 53 tools, adding a C++23 structured WSL backend with explicit registered distribution, Linux paths, literal argv/stdin, distro-owned path mapping and honest bridge/guest deadline distinctions. Real Ubuntu-on-Windows qualification passed, and both Windows/Linux SDK fixtures passed with the 53-tool inventory. The schema test caught and corrected an inherited Windows working-directory default; WSL now requires the intended Linux directory explicitly. Scope and platform limitations are in `docs/STRUCTURED_WSL.md`.

The macOS snapshot candidate now canonicalises trusted system parent aliases before SQLite NOFOLLOW opens, matching the existing live-store handling. Notification creation retains strict identity/permission checks and adds bounded missing/interrupted-open retries. Windows and Linux targeted checks passed; the macOS root cause and hosted confirmation remain open.


## Remaining lifecycle and lease integration

Artifact upload assembly/publish and legacy task checkpoints now use the bounded filesystem worker. The frontend validates the indexed layout and starts its independent coordinator; the artifact worker only connects to that existing owner. A killed worker retains the upload receipts and uncertain-publish reconciliation semantics.

Scheduler leases now have fixed, indexed records per root and slot. Admission records the exact file token/process instance before returning a lease. Release cannot update a replacement owner; recovery scans at most 64 indexed active records per page with a continuation cursor. File claims are still the sole admission authority for compatibility with existing runners. The metadata index never admits effects, fabricates completed work, or releases a live file owner.

Windows targeted CTests, both legacy/SQLite engine SDK profiles and the actual LPAC run/restart fixture passed. See `docs/audit-evidence/H06-H08-integration-local.json`. The Windows ASan lane passed at 8e43fcc after instrumenting its dependency graph; that run still requires the remaining native jobs and a retry of the macOS hosted DNS download failure. Later changes need their own complete qualification.


## Matched measurements and production drain

The H03 fixture completed 24,000 observations: four short-operation classes, three runs, 1,000 observations per class/path/run, and separate local and temporary-tunnel paths. All requests used fixed synthetic routes behind an authenticated proxy; production received no stress traffic. Timing samples, build identity, uncertainty ranges and limitations are retained in `docs/audit-evidence/H03-*`. The temporary tunnel's DNS resolver was explicitly scoped to that owned test endpoint; TLS verification remained enabled.

The allocator experiment was rejected as a default change: although its allocation-heavy component improved, two of six service pairs regressed and the confidence interval included zero. The opt-in prototype remains available for reproducible qualification; default allocation counters remain exact and global. The retained observer change reuses bounded vector/string buffers and coalesces operational logging for up to five milliseconds. Six CPU-pinned alternating service pairs (120,000 requests) all improved service CPU use, averaging 9.94%, with a descriptive paired 95% interval of 6.35-13.54%. Every one of 120,240 start/finish pairs including warmups was accounted for. Security-audit writes remain synchronous and separate. No general task-latency or hardware-universal gain is asserted.

Synchronous indexing of every transient CPU slot was measured and rejected as the ordinary path. Foreground slots retain their bounded atomic file index; durable/background and explicit resource reservations additionally require SQLite. With SQLite configured, three 1,000-observation rounds measured foreground p50 1.35-1.40ms and p95 1.69-1.78ms. The additional durability cost is reported separately. Cancellation/deadline checks after metadata admission prevent late commits from dispatching expired work; abandonment signals other waiters. Index-release acknowledgement loss cannot erase a successful command result or release a replacement owner; pending reconciliation is visible.

The C++ operator CLI `--admission drain|resume|status` provides a persisted admission generation, acknowledged in local metadata and tool status. It denies new work while preserving observation/cancellation, survives frontend replacement, and fails closed on unreadable control state. It does not imply that existing jobs or model rounds stopped: operators must reconcile them before migration. The local canary used the exact currently serving production binary as an isolated reference, verified receipt replay into the candidate before and after SQLite migration, preserved an acknowledged runner across frontend replacement, verified private snapshot fencing, rejected legacy-backend rollback after migration, and restored the same compatible artifact after a failed startup. No production state was modified. The final signed artifact must repeat these checks before promotion.

## Windows native input qualification

Run 35871838605 passed 37 of 38 Windows native suites but failed the direct CUA fixture's immediate Unicode text assertion; the same broker-driven fixture passed. The fixture now observes actual edit focus before typing and waits up to two seconds for the exact expected UTF-16 content after input. It never resends an input effect, and wrong content still fails with the observed code units. This separates a queued-input acknowledgement from application message processing; the original log did not retain the mismatched text, so it cannot prove the exact timing cause. Runtime input behavior is unchanged. Windows qualification additionally repeats both native CUA suites five times with fail-on-first-error semantics on the owned hosted desktop. The local build and noninteractive contract test passed; hosted interactive qualification is required.
