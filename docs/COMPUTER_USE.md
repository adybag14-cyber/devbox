...
- Passive `WS_EX_NOACTIVATE | WS_EX_TRANSPARENT` overlays from the target process, such as Chromium tooltip windows without an owner HWND, remain visible in its screenshot. Other-process windows and same-process interactive overlays still block capture. Mouse point-ownership checks are unchanged; this exception does not authorize clicking an overlay or another window.
