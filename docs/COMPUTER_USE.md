...
- Non-activating `WS_POPUP` windows from the target process that are marked `WS_EX_TOOLWINDOW` or `WS_EX_TRANSPARENT`, such as Chromium tooltips and colour pickers without an owner HWND, are part of the foreground app's visible UI. They may be observed and targeted within the returned image. Independent windows without these transient-popup flags and all other-process overlays still block capture. Pointer hit-testing must resolve to the target window, an owned popup, or a qualifying popup with the same PID; window/process identity, bounds, foreground and desktop checks still apply.
