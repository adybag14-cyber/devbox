...
class RunController {
    ...
    // True only when this invocation committed the transition. Admission callers
    // use this receipt to distinguish fresh reservations from stopped no-ops.
    bool save(StateRecord& record, std::string_view event);
