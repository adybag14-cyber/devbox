...
int run(int argc, char** argv) {
    ...
    try {
        ...
        }
        {
            ...
            require(denied, "server proof prevents accepting an impostor acknowledgement");
            {
                ...
                require(!impostor->captured.empty() &&
                        ...
                        "intercepted IPC discloses neither state payload nor channel credentials");
            }
            fake.stop();
        }
        ...
        require(one->get("run", "shared")->revision == 1 && one->generation() > generation,
                "client reconnect restarts the writer with a newer fence and retained data");
        require(two->apply_once("same-batch", {&value, 1}),
                "cross-restart receipt replay cannot duplicate an admitted transaction");
        require(stop_state_coordinator(directory), "restarted coordinator reaches terminal stop");
        std::cout << "Encrypted authenticated IPC, single writer, frontend survival, spoof refusal and "
